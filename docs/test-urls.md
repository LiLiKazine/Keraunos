...
| YouTube | `https://youtu.be/WuMlsfKeWHc?is=UvPuvc7agzS-LGeP` | ✅ device (2026.6.9 + JSC provider) | **Confirmed working on-device**: the new `KeraunosJavaScriptCoreJCP` solves n/sig by running yt-dlp's EJS bundle in JavaScriptCore. First cold run timed out (heavy EJS solve + PoT mint > 30s watchdog); warm retry succeeded — now auto-retried (timeout added to the cold-start retry). >1080p gated on the SABR spike. |
